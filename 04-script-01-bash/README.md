...
| ------------- | ------------- | ------------- |
| `c`  | a+b  | В данном случае выражение интерпретируется как строка |
| `d`  | 1+2  | В данном варианте значения переменных подставляются, но само выражение интерпретируется как строка |
| `e`  | 3    | В данном случае скобки используются для обозначения арифметических операций |
...
```bash
#!/usr/bin/env bash
hosts_IP=(192.168.31.1 173.194.222.113 87.250.250.242)
for IP in ${hosts_IP[@]}
    do
        for L in {1..5}
            do 
                echo -n $IP $(date "+%D %T") >> test.log
                curl "$IP:80"
                if [ $? != 0 ]
                then
...
```bash
#!/usr/bin/env bash
hosts_IP=(192.168.31.1 173.194.222.113 87.250.250.241)
while ((1==1))
    do
        for IP in ${hosts_IP[@]}
            do
