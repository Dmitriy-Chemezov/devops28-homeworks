...
1. Команда `vagrant ssh` из директории, в которой содержится Vagrantfile, позволит вам оказаться внутри виртуальной машины без дополнительных настроек. Попрактикуйтесь в выполнении обсуждаемых команд в терминале Ubuntu.
	![Подключение через команду vagrant ssh](https://github.com/Dmitriy-Chemezov/devops28-homeworks/blob/main/03-sysadmin-01-terminal/2.png)
...
1. Изучите разделы `man bash`, почитайте о настройках самого bash:
    ...
    * что делает директива `ignoreboth` в bash?
	...
	> Длину журнала history можно задать при помощи переменной HISTSIZE, строка в man - 895.
	> Директива `ignoreboth` для переменной HISTCONTROL означает, что команды с начальным пробелом и дубликаты команд не будут сохраняться в истории
	 
6. В каких сценариях использования применимы скобки `{}`, на какой строчке `man bash` это описано?
	
	> {} Зарезервированные слова - это слова, которые имеют особое значение для оболочки. (строка в man 180).
	> {} Обозначяет список, который выполняется в среде текущего командного интерпретатора(строка в man 267).  
...
3. С учётом ответа на предыдущий вопрос подумайте, как создать однократным вызовом `touch` 100 000 файлов. Получится ли аналогичным образом создать 300 000 файлов? Если нет, то объясните, почему.
	...
	> Создать однократным вызовом `touch` 100 000 файлов возможно. touch {000001..100000}.txt
	> Создание однократным вызовом `touch` 300 000 файлов вызывает ошибку -bash: /usr/bin/touch: Argument list too long

4. В man bash поищите по `/\[\[`. Что делает конструкция `[[ -d /tmp ]]`?

	> квадратные скобки в них заключаеться выражение, возвращает 0 или 1 проверяет условие. [[ -d /tmp ]] проверяет, явлется-ли tmp директорией.
...
1. Чем отличается планирование команд с помощью `batch` и `at`?
	...
	> Команда batch — для выполнения одноразовых задач, которые должны выполняться, когда загрузка системы становится меньше 1,5
	> at - выполняет комманды в указанное время
