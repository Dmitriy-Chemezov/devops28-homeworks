...
CREATE TABLE
test_database=# CREATE RULE rule_orders_1 AS ON INSERT TO orders WHERE (price > 499) DO INSTEAD INSERT INTO orders_1 VALUES (NEW.*);
CREATE RULE
test_database=# CREATE RULE rule_orders_2 AS ON INSERT TO orders WHERE (price <= 499) DO INSTEAD INSERT INTO orders_2 VALUES (NEW.*);
CREATE RULE
...
- Да, можно было изначально сделать таблицу на основании Декларативного секционирования (Declarative Partitioning), т.е. разбить таблицу на необходимое количество секций. При этом основная таблица в выводе \dt+  указывается как "секционированная таблица".
